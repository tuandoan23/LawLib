...
public class DuThaoLuatFragment extends BaseFragment implements LawContract.LawListView {
    ...
    @BindView(R.id.rvDuThaoLuat)
    RecyclerView rvDuThaoLuat;

    @Override
    public int initLayout() {
        return R.layout.fragment_duthaoluat;
    }
    ...
    public void createAdapter() {
        ...
        if (orientation == ORIENTATION_PORTRAIT) {
            ...
            rvDuThaoLuat.setLayoutManager(linearLayoutManager);
        } else if (orientation == ORIENTATION_LANDSCAPE){
            gridLayoutManager = new GridLayoutManager(this.getContext(), 2);
            rvDuThaoLuat.setLayoutManager(gridLayoutManager);
        }
        lawsAdapter = new LawsAdapter(laws, this.getContext());
        rvDuThaoLuat.setAdapter(lawsAdapter);
    }

    @Override
    public void loadData() {
        presenter.getLawListByType(this.getContext(), Utils.DU_THAO_LUAT);
    }
