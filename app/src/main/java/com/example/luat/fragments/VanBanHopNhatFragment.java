...
public class VanBanHopNhatFragment extends BaseFragment implements LawContract.LawListView {
    ...
    @BindView(R.id.rvVBHopNhat)
    RecyclerView rvVBHopNhat;

    @Override
    public int initLayout() {
        return R.layout.fragment_vbhopnhat;
    }
    ...
    public void createAdapter() {
        ...
        if (orientation == ORIENTATION_PORTRAIT) {
            ...
            rvVBHopNhat.setLayoutManager(linearLayoutManager);
        } else if (orientation == ORIENTATION_LANDSCAPE){
            gridLayoutManager = new GridLayoutManager(this.getContext(), 2);
            rvVBHopNhat.setLayoutManager(gridLayoutManager);
        }
        lawsAdapter = new LawsAdapter(laws, this.getContext());
        rvVBHopNhat.setAdapter(lawsAdapter);
    }
