...
public class LuatSuaDoiFragment extends BaseFragment implements LawContract.LawListView {
    ...
    @BindView(R.id.rvLuatSuaDoi)
    RecyclerView rvLuatSuaDoi;

    @Override
    public int initLayout() {
        return R.layout.fragment_luatsuadoi;
    }
    ...
    public void createAdapter() {
        ...
        if (orientation == ORIENTATION_PORTRAIT) {
            ...
            rvLuatSuaDoi.setLayoutManager(linearLayoutManager);
        } else if (orientation == ORIENTATION_LANDSCAPE){
            gridLayoutManager = new GridLayoutManager(this.getContext(), 2);
            rvLuatSuaDoi.setLayoutManager(gridLayoutManager);
        }
        lawsAdapter = new LawsAdapter(laws, this.getContext());
        rvLuatSuaDoi.setAdapter(lawsAdapter);
    }

    @Override
    public void loadData() {
        presenter.getLawListByType(this.getContext(), Utils.LUAT_SUA_DOI);
    }
