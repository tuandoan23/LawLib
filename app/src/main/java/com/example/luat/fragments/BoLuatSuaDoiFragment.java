...
public class BoLuatSuaDoiFragment extends BaseFragment implements LawContract.LawListView {
    ...
    @BindView(R.id.rvBoLuatSuaDoi)
    RecyclerView rvBoLuatSuaDoi;

    @Override
    public int initLayout() {
        return R.layout.fragment_boluatsuadoi;
    }
    ...
    public void createAdapter() {
        ...
        if (orientation == ORIENTATION_PORTRAIT) {
            ...
            rvBoLuatSuaDoi.setLayoutManager(linearLayoutManager);
        } else if (orientation == ORIENTATION_LANDSCAPE){
            gridLayoutManager = new GridLayoutManager(this.getContext(), 2);
            rvBoLuatSuaDoi.setLayoutManager(gridLayoutManager);
        }
        lawsAdapter = new LawsAdapter(laws, this.getContext());
        rvBoLuatSuaDoi.setAdapter(lawsAdapter);
    }

    @Override
    public void loadData() {
        presenter.getLawListByType(this.getContext(), Utils.BO_LUAT_SUA_DOI);
    }
