...
public class NghiDinhSuaDoiFragment extends BaseFragment implements LawContract.LawListView {
    ...
    @BindView(R.id.rvNghiDinhSuaDoi)
    RecyclerView rvNghiDinhSuaDoi;

    @Override
    public int initLayout() {
        return R.layout.fragment_nghidinhsuadoi;
    }
    ...
    public void createAdapter() {
        ...
        if (orientation == ORIENTATION_PORTRAIT) {
            ...
            rvNghiDinhSuaDoi.setLayoutManager(linearLayoutManager);
        } else if (orientation == ORIENTATION_LANDSCAPE){
            gridLayoutManager = new GridLayoutManager(this.getContext(), 2);
            rvNghiDinhSuaDoi.setLayoutManager(gridLayoutManager);
        }
        lawsAdapter = new LawsAdapter(laws, this.getContext());
        rvNghiDinhSuaDoi.setAdapter(lawsAdapter);
    }

    @Override
    public void loadData() {
        presenter.getLawListByType(this.getContext(), Utils.NGHI_DINH_SUA_DOI);
    }
